...
const UVC_G3_MICRO = 'UVC G3 Micro';
...
class UfvApi extends EventEmitter {
    ...
    GetRecordings(startTime=0, endTime = 0, recordingId = ''){
        return new Promise((resolve, reject) => {
            let params = {
            };
            ...
            this._get(`recording/${recordingId}`, params)
                .then(response => {
                    let result = JSON.parse(response).data;
                    ...
                    if (result) {
                        ...
                        reject('Error obtaining recordings.');
                    }
                })
                .catch(error => reject(error));
        });
    ...
    DownloadRecording(recordingId){
        return new Promise((resolve, reject) => {
            this.GetRecordings(0,0,recordingId)
                .then(rec => {
                    this._download(`recording/${recordingId}/download`)
                    .then(buffer => {
                        rec.buffer = buffer;
                        resolve(rec)
                    })
                    .catch(error => reject('Error obtaining recording buffer: ' + error));
                })
            .catch(() => reject(`No recording found: ${recordingId}`));            
        });
    }
